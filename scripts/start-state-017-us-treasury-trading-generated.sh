...
export GRAFANA_PORT
COMPOSE_DIR="${GENERATED_ROOT}/code/target-generated/us-treasury-trading"
...
if (( DRY_RUN == 1 )); then
  ...
  echo "[done] dry run complete for state 017"
  exit 0
fi
...
wait_for_http "reference-data" "http://localhost:18085/instruments" || exit 1
...
bash "${REPO_ROOT}/scripts/start-grafana-traderx-dashboards.sh" \
  ...
  "TraderX" \
  "traderx-obs-017-us-treasury-trading" || true

echo "[done] state 017 U.S. Treasury trading runtime started"
echo "[ui] http://localhost:8080"
echo "[api-explorer] http://localhost:8080/api/docs"
echo "[instruments] http://localhost:18085/instruments"
